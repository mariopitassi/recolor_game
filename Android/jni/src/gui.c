 ...
 * puts all GUI colors in and then pick some colors from an array for game cells.
 *
...
static Gui_color *init_colors(SDL_Renderer *ren, game g) {
  ...
  for (uint x = 0; x < nb_col; x++) cells[x] = col_arr[x];
 ...
 */
static void render_static_textures(SDL_Renderer *ren, Env *env){
  SDL_Rect rect;

  // Set Titles's position and size then renders it
  rect.w = env->grid_width/2;
  rect.h = (env->win_height - env->grid_height)/2;
  ...
  rect.w = env->grid_width/8;
  rect.h = (env->win_height - env->grid_height)/4;
  ...
  SDL_RenderDrawLine(ren, env->grid_start_x + env->grid_width/4, env->grid_start_y + env->grid_height, env->grid_start_x + env->grid_width/4, env->win_height);
  SDL_RenderDrawLine(ren, env->grid_start_x + (3*env->grid_width/4), env->grid_start_y + env->grid_height, env->grid_start_x + (3*env->grid_width/4), env->win_height);
  //SDL_RenderDrawLine(ren, env->grid_start_x, env->grid_start_y + env->grid_height, env->grid_start_x, env->win_height);
  //SDL_RenderDrawLine(ren, env->grid_start_x + env->grid_width, env->grid_start_y + env->grid_height, env->grid_start_x + env->grid_width, env->win_height);
}

// To optimize -- better not create and destroy texture in renderer
static void render_status(SDL_Renderer *ren, Env *env){
  SDL_Rect rect;

  // Convert (nb_cur / nb_moves_max) to string.
  char str_status[15];
  sprintf(str_status, "%u/%u", game_nb_moves_cur(env->game), game_nb_moves_max(env->game));
  ...
  SDL_Surface * surface = TTF_RenderText_Blended(font, str_status, c);
...
Env *init(SDL_Window *win, SDL_Renderer *ren, int argc, char *argv[]) {
  ...
  SDL_Color c = {105, 105, 105, 255}; /* dim_grey color in RGBA */
  TTF_Font * font = TTF_OpenFont(FONT, FONTSIZE);
  if(!font) ERROR("TTF_OpenFont: %s\n", FONT);
  TTF_SetFontStyle(font, TTF_STYLE_NORMAL); // TTF_STYLE_ITALIC | TTF_STYLE_NORMAL

  SDL_Surface * title = TTF_RenderText_Blended(font, "Flood It", c);
  env->textures[TITLE] = SDL_CreateTextureFromSurface(ren, title);

  SDL_Surface * restart = TTF_RenderText_Blended(font, "Restart", c);
  env->textures[RESTART] = SDL_CreateTextureFromSurface(ren, restart);

  SDL_Surface * quit = TTF_RenderText_Blended(font, "Quit", c);
  env->textures[QUIT] = SDL_CreateTextureFromSurface(ren, quit);

  SDL_Surface * won = TTF_RenderText_Blended(font, "YOU WON !", c);
...
void render(SDL_Window *win, SDL_Renderer *ren, Env *env) {
  ...
  render_static_textures(ren, env);

  // Render grid
  draw_grid(win, ren, env);

  // Render nb_cur / nb_max (TO OPTIMIZE !)
...
bool process(SDL_Window *win, SDL_Renderer *ren, Env *env, SDL_Event *e) {
  ...
  if (e->type == SDL_FINGERDOWN) {
    ...
    if (click_on_grid(e, env) && env->game_state == PLAYING) {
      ...
      // Tapped on 'Quit'
      else if (button_x > env->grid_start_x + (3*env->grid_width/4) && button_x < env->grid_start_x + env->grid_width)
        return true;
    }
  }
#else
