...
bool test_game_new() {
  ...
  for (int i = 0; i <= SIZE - 1; i++) {
    for (int j = 0; j <= SIZE - 1; j++) {
      if (game_cell_current_color(g, j, i) != tab[12 * i + j]) {
        fprintf(stderr,
                "Error : Les cellules ne sont pas initialisées avec les bonnes "
                "valeurs du tableau.");
        game_delete(g);
        return false;
      }
  ...
  if (coups_max != coups_test) {
    fprintf(stderr, "Error : Le nombre de coups max ne coïncide pas : %d != %d",
            coups_max, coups_test);
    game_delete(g);
    return false;
  }
  ...
  if (coups_current_test != 0) {
    fprintf(
        stderr,
        "Error : Le nombre initial de coups joué ne coïncide pas : %d != %d",
        coups_current_test, 0);
    game_delete(g);
    return false;
  }
...
bool test_set_max_moves() {
  ...
  if (game_nb_moves_cur(g) > coups) {
    fprintf(stderr,
            "Error : Impossible de définir un coups_max = %d < coups_cur =  %d",
            coups, game_nb_moves_cur(g));
    game_delete(g);
    return false;
  }
  ...
  if (coups_test2 != coups) {
    fprintf(stderr,
            "Error : Le nombre de coups max ne coïncide pas sur game_new_empty "
            ": %d != %d",
            coups, coups_test2);
    game_delete(g2);
    return false;
...
bool test_copy() {
  ...
  if (g == NULL) {
    fprintf(stderr,
            "Error : Partie d'origine non initialisée : pas assez de mémoire");
    return false;
  }
  ...
  if (g2 == NULL) {
    fprintf(stderr,
            "Error : Partie copiée non initialisé : pas assez de mémoire");
    return false;
  }
  ...
  for (int i = 0; i <= SIZE - 1; i++) {
    for (int j = 0; j <= SIZE - 1; j++) {
      if (game_cell_current_color(g, j, i) !=
        ...
        fprintf(stderr, "Error : Les cellules copiées ne sont pas initialisées "
                        "correctement.");
        game_delete(g);
        game_delete(g2);
        return false;
  ...
  if (game_nb_moves_cur(g) != game_nb_moves_cur(g2)) {
    fprintf(stderr, "Error : Le nombre de coups joué est différent");
    game_delete(g);
    game_delete(g2);
    return false;
  }
