 ...
 *
 * @return true if all the game structure's fields have been correctly initialised
 * @return false if a field of the game structure hasn't been correctly initialised
 */
 ...
 *
 * @return true if all the game structure's fields have been correctly initialised
 * @return false if a field of the game structure hasn't been correctly initialised or the game hasn't been created
 */
bool test_game_new_empty_ext(){
  ...
  if (game_width(g) != width){
    fprintf(stderr,"Erreur, la largeur du tableau du jeu n'a pas été initialisé correctement");
    game_delete(g);
    return false;
  }
  ...
  //checking if the max nb moves has been initialized to 0
  if (game_nb_moves_max(g) != 0){
    fprintf(stderr,"Erreur, le nombres de moves max n'a pas été initialisé correctement à 0");
    game_delete(g);
    return false;
  }
  //checking if the wrapping bool has been correctly initialized
  if (game_is_wrapping(g) != wrapping){
    fprintf(stderr,"Erreur, le booléen determinant si le jeu est wrapping ou non n'a pas été initialisé correctement");
    game_delete(g);
    return false;
  }
  //free the created game
...
/**
 * @brief Tests if the current nb_moves has been correctly initialized to 0 at the begginning of a game
 and then tests if the current nb_moves is correctly incremented after playing a move
 * 
 * @return true if the current nb_moves is correctly initialized and incremented, also when the game is restarted
 * @return false if the game hasn't been correctly initialized or incremented. Also return false if the game doesn't exist
 */
bool test_game_nb_moves_cur() {
  ...
  for (int i = 0; i <= coups_max;) {
    ...
    i++;
  //cheking if the current nb moves is correctly incremented after playing a move
